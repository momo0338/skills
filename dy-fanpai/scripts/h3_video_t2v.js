...
// 前置：ComfyUI 以 --enable-cors-header=* 启动（浏览器直连必需）；
//       模型依赖 download_minimax_h3.sh：fl2va unet + qwen3vl_32b clip + 双 VAE
// 注意：API Key 填任意占位值（ComfyUI 无鉴权，但无限画布校验非空）
// 输入：prompt（提示词）、params.seconds（时长）、params.size（WxH 比例）
...
const workflow = {
  ...
  "105:15": {
    "inputs": { "noise_seed": 997100332868374 },
...
workflow["105:107"].inputs.expression = `max(5, round(a * 24)) + (5 - (max(5, round(a * 24)) % 17)) % 17`; // 帧数对齐 17
workflow["115"].inputs.aspect_ratio = aspectRatio;                // 比例
workflow["115"].inputs.megapixels = 0.4;                          // 0.4MP（实测稳定档）
