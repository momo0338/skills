...
```text
新项目路径：/Users/zhugx/src/dy-fanpai
仓库可见性：初期私有
Python：3.12
包管理：uv，同时保证 pip 可安装
CLI：argparse
...
启动步骤：创建 `/Users/zhugx/src/dy-fanpai`、执行本地 `git init`、复制本文件为根目录 `EXECUTION_PLAN.md` 并校验哈希；首次提交前等待项目负责人提供 repo-local Git 身份。随后只读原项目，产出：
...
新项目 /Users/zhugx/src/dy-fanpai 独立实现。
...
原项目 /Users/zhugx/src/daihuo-fanpai 只读。
新项目目标路径 /Users/zhugx/src/dy-fanpai。
