...
keywords:
  ...
  - 爱企查
env:
  - FN_API_KEY  # required，个人 API Key
security:
  child_process: false
  eval: false
...
**设置**：使用前需要先在风鸟 Web 端生成个人 API Key，并配置为环境变量 `FN_API_KEY`。完整 Key 仅在创建时展示一次，请妥善保存。实际请求通过 URL 参数 `apikey` 传递，不通过 Headers 传递。

**额度说明**：多个 API Key 共用同一账号的每日额度，创建多个 Key 不会增加每日可用次数。当前剩余额度以风鸟 Web 端 API Key 页面展示为准。
...
- `code=9999` 且提示 API Key 未配置或无效：引导用户在风鸟 Web 端生成个人 API Key，并配置环境变量 `FN_API_KEY`
- `code=9999` 且 `msg` 包含“访问已达上限”：告知用户当前账号当日额度已用完，并引导其查看 API Key 页面实时额度
