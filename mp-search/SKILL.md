...
1. **关键词极速检索（Search）** ✅：通过命令行极速检索全网微信公众号文章，支持「最热」综合排序与「最新」时效排序，免登录、零等待，直接输出标准化 JSON 数据与 Markdown 对比表格，为下游推文采写（如 `gh-write`）提供一手竞品与讨论现状依据。
...
- `-n, --limit`：返回条数（默认 10）
- `-o, --output`：输出标准化 JSON 文件路径（包含 keyword、rank_type 与 items 列表）
