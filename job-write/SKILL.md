...
> ⛔ **2026-09-17 朱总裁定 —— 母版是唯一底版**：**A 线（校招）与 B 线（事业编招考）一律使用
> 「2027 校园招聘官方标准母版 + 左上角主体 Logo 胶囊」**，不得自创封面底版。
>
> 背景留档：我曾以「母版画面烘焙着『2027 秋季校招开启』，与 B 线『2026 年毕业生 + 社会人员』
> 的编制招考受众不符」为由，给栖霞稿另做了一版「主体主色专属封面」（见 §7.5 存档），**被否决**。
> **结论：视觉一致性优先于文案贴合——不要自作主张换底版。** 若确需改母版文案，须先问朱总。

全专栏统一采用用户指定的 **2027 校园招聘官方标准母版**（清新活力蓝绿网格微光 + 磨砂玻璃通透质感卡片）。
...
### 7.4 生成脚本（母版 + logo 胶囊，可直接复用）

> 栖霞实例（2026-09-17 实测通过）——政府/单位 logo 若是**横向长条**（国徽 + 多行文字如
> 「南京市栖霞区人民政府 / 南京经济技术开发区管理委员会 / 南京市仙林大学城管理委员会」），
> 直接按 42px 高缩放即可塞进胶囊，**无需裁切**：

```python
from PIL import Image, ImageDraw
master = Image.open(f"{BASE}/2027年校园招聘/素材/标准校招封面母版.png").convert("RGBA")  # 900×383
...
### 7.5 ❌ 已否决方案存档：B 线「主体主色专属封面」（不要再走）

2026-09-17 我曾为栖霞稿做了一版**弃用母版、改主体主色（栖霞红 `#C8161D`）自制底版**的封面
（红渐变 + 网格 + 柔光圆斑 + 上部安全区文案 + 右上主题药丸），**当日被朱总否决**。

- **否决理由**：全专栏封面视觉一致性优先于「文案贴合度」；母版即唯一底版。
- **工具留存**：`03-工作记录/码上职业/工具/build_bl_cover.py`（参数化生成器，**当前停用**）。
  存档它的唯一价值是——若将来朱总**主动**要求换底版，可直接复用，不必重写。
- **不要重犯**：发现母版文案（「2027 秋季校招开启」）与内容受众不符时，**正确动作是问朱总**，
  不是自己换底版。同类「为追求更贴切而偏离母版」的改动一律先问。
...
- [ ] **封面 = 标准校招母版 + 左上角主体 Logo 胶囊**（胶囊 (40,32)、logo 高 42px；**不得自创底版**）？
