...
6. **SEO 摘要规范（digest，必产）**：
   ...
   - **存放**：有 `-源稿.md` 的写在 h1 之后一行：`> **SEO 摘要**（NNN字，无 emoji）：` + `> <摘要正文>`（流水线自动提取，日志须显示「摘要(手动)」）；无源稿的用 `--digest` 传参。
...
## 七、封面规范（用户标准母版 + 左上角企业 Logo 胶囊）

专栏统一采用用户指定的 **2027 校园招聘官方标准母版**（清新活力蓝绿网格微光 + 磨砂玻璃通透质感卡片）。
