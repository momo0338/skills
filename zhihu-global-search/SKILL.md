...
`{baseDir}` 是 agent 框架在运行时自动替换的变量，指向当前 skill 目录的绝对路径（即 `skills/zhihu-global-search/`）。

```bash
python3 {baseDir}/scripts/zhihu-global-search.py '{"query":"如何理解 rave 文化","count":8,"filter":"host==\"example.com\"","search_db":"all"}'
```
